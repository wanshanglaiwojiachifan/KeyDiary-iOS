...
- (void)getUserDiaryCallback:(NSDictionary *)diaryInfo  code:(NSInteger)code
{
    ...
    NSLog(@"getDiaryCallback diaries %@", [data valueForKeyPath:@"diaries"]);
...
- (void)importToCoreData:(NSArray *)dataFromService
{
    ...
    NSArray *allDiary = [self getAllDiary];
    NSMutableDictionary *diaryMapLocal = [NSMutableDictionary dictionaryWithCapacity:[allDiary count]];
    for (int j = 0; j < [allDiary count]; j++) {
        [diaryMapLocal setValue:[allDiary objectAtIndex:j] forKey:[[allDiary objectAtIndex:j] valueForKey:@"dateStr"]];
    }
    ...
    for (int i = 0; i < [dataFromService count]; i++) {
        ...
        if (!singleCoreData) {
            ...
            //singleCoreData = [existDiary objectAtIndex:0];
            if ([[singleCoreData valueForKey:@"update"] isEqualToString:@"0"]) {
                if (![[singleCoreData valueForKey:@"keyword"] isEqualToString:[singleData valueForKey:@"content"]]) {
                    [self insertSingleDiary:[singleData valueForKeyPath:@"d"] keyword:[singleData valueForKeyPath:@"content"] updateTime:[self strToDateByFormat:[singleData valueForKeyPath:@"created"] format:@"yyyy-MM-dd HH:mm:ss"] context:context];
                    count++;
                }
            } else {
